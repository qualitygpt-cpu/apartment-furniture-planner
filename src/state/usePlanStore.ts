...
const defaultLayers: LayerState = { grid:true, rooms:true, walls:true, openings:true, fixedElements:true, furniture:true, labels:true, dimensions:true, clearance:true, validation:true };
...
export const usePlanStore = create<Store>((set) => ({
  plan: null, errors: [], layers: defaultLayers,
  setPlan: (input) => { const {plan,errors}=validatePlan(input); set({ plan: plan ?? null, errors }); localStorage.setItem('apartment-plan-autosave', JSON.stringify(input)); },
